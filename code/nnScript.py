...
def preprocess():
     ...
     temp_data = np.concatenate((train_data, validation_data, test_data))
     
     duplicate_flag = np.all(temp_data == temp_data[0,:], axis=0)
     duplicate_cols = np.where(duplicate_flag == True)
     global selected_features
     selected_features = np.where(duplicate_flag != True)[0]
     
     temp_data = np.delete(temp_data, duplicate_cols[0], axis=1)
     print("Features:", temp_data.shape[1])
     train_data = temp_data[0:len(train_data),:]
     validation_data = temp_data[len(train_data) : (len(train_data)+len(validation_data)),:]
     test_data = temp_data[(len(train_data)+len(validation_data)) : (len(train_data)+len(validation_data) + len(test_data)),:]
